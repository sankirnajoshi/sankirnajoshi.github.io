...
#print "Standard deviation of prices with pandas: ${:,.2f}".format(std_price1)
```
...
![png](boston_housing_files/boston_housing_7_0.png)



![png](boston_housing_files/boston_housing_7_1.png)



![png](boston_housing_files/boston_housing_7_2.png)
...
- As a first intuition I do not think that the 'PTRATIO' variable will have much impact on the home prices. But if this means that there are less schools in areas with <s>lower</s> higher 'PTRATIO' then the home prices in neighborhood with less schools or <s>lower</s> 'PTRATIO' will be lower that neighborhoods with higher 'PTRATIO'.
...
![png](boston_housing_files/boston_housing_22_0.png)
...
- The max_depth of the model on the <s>upper-left</s> lower-left corner is 6.
...
![png](boston_housing_files/boston_housing_26_0.png)
...
**Answer: **
[Updated]
- In K-Fold cross validation, the training data is shuffled and split into k bins and model is trained k times, where every k times time a new bin is used for validating the fit and rest k-1 bins are used for training.
- When used along with grid search, if we tune hyperparameters on the test set directly, with every optimization of parameters we are overfitting the test set and this cannot generalize well. To avoid this, we use cross validation along with grid search to tune the hyper parameters and find the best params on the validation set and then predict on the test set.
...
- Parameter 'max_depth' is 4 for the optimal model.
- My guess in Question 6 above, was max_depth = 3. I had suspected that the model might overfit if we increase the max depth to 4.
